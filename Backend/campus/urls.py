...
urlpatterns = [
    ...
    path('building/<int:building_pk>'), # 특정 빌딩 세부사항 + 해당 빌딩 내 모든 층 목록 조회, 빌딩 수정 및 삭제
    path('building/<int:building_pk>/floor/', ),  # 특정 빌딩 내 층 추가
    
    path('floor/<int:floor_pk>'), # 특정 층 세부사항 + 모든 쓰레게통 조회, 층 수정 및 삭제 
    path('floor/<int:floor_pk>/trashbin/',), # 특정 층 내 쓰레기통 추가
    
    path('trashbin/<int:trashbin_pk>', ), # 특정 쓰레기통 조회, 수정 및 삭제
